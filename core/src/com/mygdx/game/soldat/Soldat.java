...
import com.mygdx.game.particle.blood;
...
public abstract class Soldat implements Serializable {
    ...
    public void update(){
        center_render();
        render_metod.transorm_img(this.x_rend,this.y_rend,this.width_render,this.height_render,this.rotation+180,this.soldat_image);
    }
    ...
    public void clear(ArrayList<Soldat>obj,int i){
        if(this.clear_sost == 1){
            ...
            Main.liquid_obj.add(new blood(this.x+i1,this.y));}
            obj.remove(i);
        }
    }
    public void fire_bull(double g,double g_left,double g_right){
        ...
        if(this.time < 0 && g>g_left && g<g_right){
            ...
            Main.bull_obj.add(new bull_tank(this.x,this.y,-this.rotation+180,this.damage,this.penetration,this.team,(byte)1));
        }
    }
    public void fire_flame(double g,double g_left,double g_right){
        ...
        if(this.time < 0 && g>g_left && g<g_right){
            this.time = this.time_max;
            Main.bull_obj.add(new bull_flame(this.x,this.y,-this.rotation+ rand.rand(-15,15)+180,this.damage,this.t_damage,this.penetration,this.team,(byte)1));
            Main.bull_obj.add(new bull_flame(this.x,this.y,-this.rotation+ rand.rand(-15,15)+180,this.damage,this.t_damage,this.penetration,this.team,(byte)1));
        }
    }
    public void fire_mortar(double g,double g_left,double g_right){
        ...
        if(this.time < 0 && g>g_left && g<g_right){
            this.time = this.time_max;
            Main.bull_obj.add(new bull_fragmentation(this.x,this.y,-this.rotation+180,this.damage,this.penetration,this.damage_fragmentation,
    ...
    public void move_soldat_ii_bull(int i){
        if(Main.player_obj.size() != 0) {
            int[] sp = metod.detection_near_soldat_transport_i_def(Main.soldat_obj, i, Main.player_obj);
